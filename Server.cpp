...
int main(int argc, char* argv[]){
	...
	while(1){
		while(clients < MAX_CLI){
			if(cli_desc = accept(serv_sock_desc, (struct sockaddr*)&client, (socklen_t*)&c)){
				if(pthread_create(&thread1, NULL, transmission_handler, (void*)&cli_desc) < 0){
...
void* transmission_handler(void* cli_desc1){
	...
	//momkene aval 2 bere bad 3 ham bere bad 4 beran tu ghesmate 1 va 3 , 4 bahan kanect beshan
	...
	curPrior = newPrior;
		if(clients != 0){
			pthread_cond_wait(&thr_end_ctrler, &thr_ending_waiter);//-------------------------
			// pthread_cond_broadcast(&thr_end_ctrler);//it let threads do the ending process
			// pthread_mutex_lock(&thr_ending_waiter);
		}
	clients++;
	}
    ...
    while(prior == curPrior){
    	while((cliMsgSize = recv(cli_desc , cliMsg , R_BUF_SIZE , MSG_DONTWAIT)) > 0 )
    	{
			cliMsg[cliMsgSize] = '\0';
